...
void setup() //main setup functions
{
    ...
    config.title = "SmartWaterPressureMonitor Controller"; //set title of webapp
...
void loop()
{
    ...
    if (millis() - lastPub > updateInterval) //publish data to mqtt server
    {
        mqttClient.publish(topicPressureSensorStatus, getPressure().c_str());

        mqttClient.publish(topicPressureSensorLive, getPressure().c_str());
        mqttClient.publish(topicPressureRelayLive, relayStateN.c_str());
